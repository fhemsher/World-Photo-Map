...
function trackDrawPath()
{
    ...
    if(activeElem&&ActiveElem&&ActiveElemStop==false) //   &&ActiveElemStop==false
    {
            ...
            if(RubberLine&&PausePath==false&&PathClosed==false)
        {
         ...
         commentDiv.innerHTML="Length: "+numberWithCommas(PreviousLL.distanceTo(SVGLatLng).toFixed(0))+"m<br>Direction: "+az.toFixed(3)  +" &deg;"+
         "<br>Total Length = "+numberWithCommas(TotalDistance)+"m"
...
{   TotalDistance=0
    MapLatLngClick=SVGLatLng //---start path point LL---
    PreviousLL=SVGLatLng //---distance---
...
function clickNextPathPoint()
{
    ...
    {
        TotalDistance+=+PreviousLL.distanceTo(SVGLatLng).toFixed(0)
        console.log(TotalDistance)
    }



    PreviousLL=SVGLatLng//---disance between points===
        ...
        if(DrawPathType=="basis")
    {
        ...
        getSmoothPathDistance()


    }

        var commentSplit=commentDiv.innerHTML.split("Total Length = ")[0]
        commentDiv.innerHTML=commentSplit+"Total Length = "+numberWithCommas(TotalDistance)+"m"
