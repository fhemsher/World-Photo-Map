...
function finishDrawPath()
{
    ...
    if(cw.drawPathRightAngleCheck.checked==true)
        ...
        finishedElem.setAttribute("onmousedown", "startPathDrawEdit("+id+",evt)")
         ...
         //---no transformsetLatLng(finishedElem) //---helperFuncts.js---
           MyMap.dragging.enable()
           MyMap.scrollWheelZoom.enable();
    ...
    finishedElem.style.cursor = "default"
           ...
           var svgPnt=L.point(0,0)
        var latLng=MyMap.layerPointToLatLng(svgPnt)
        var lat=latLng.lat
        var lng=latLng.lng
        finishedElem.setAttribute("lat",lat)
        finishedElem.setAttribute("lng",lng)


        var transformRequestObj=mySVG.createSVGTransform()
        var animTransformList=finishedElem.transform
        ...
        var initZoom=parseInt(finishedElem.getAttribute("InitZoom"),10)
