...
function trackDrawCircle()
{
    ...
    if(ActiveElem==null&&EditCircle==false && CircleDeleted==false&&ActiveCircleCopy==false)
    {
...
function placeDrawCircle()
{
    ...
    CircleCenterLL = MyMap.layerPointToLatLng(svgPnt)
...
function setEditCircle()
{
   ...
   // MyMap.setZoom(initZoom)
    ...
    if(ActiveElem.attr("stroke-dasharray"))
        ...
        var initZoom=+ActiveElem.attr("InitZoom")
        var scale = (Math.pow(2, mapZoom)/2)/(Math.pow(2, initZoom)/2);
             //--place dragDot----
    var radius = +ActiveElem.attr("r")*scale

    var Tfm=ActiveElem.attr("transform")
    ...
    DragDot.attr("transform", "translate("+(x+radius) +" "+y+")")
    DragDot.style("visibility", "visible")
