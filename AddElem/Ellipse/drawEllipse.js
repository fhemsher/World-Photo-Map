...
function editEllipseDraw(elemObjEdit, evt)
{
    ...
    if(isRightMB&&DrawEllipse==false)
    {
...
function setEditEllipse()
{
    ...
    if(!rotatedDeg) rotatedDeg = 0
       ...
       var mapZoom=MyMap.getZoom()
        ...
        var radiusX = parseFloat(ActiveElem.attr("rx"))//*scale
        var radiusY = parseFloat(ActiveElem.attr("ry"))//*scale
        DragDot.attr("cx", radiusX)
        DragDot.attr("cy", radiusY)
        DragDot.attr("transform", ActiveElem.attr("transform"))
        DragDot.style("visibility", "visible")
